...
import numpy as np
from stockapp.indicators import calc_ema, calc_macd, calc_rsi, get_suggestion
...
def get_short_term(stock, no_of_minutes):
    ...
    mycursor.execute("SELECT * from real_time where sid='" + str(sid) + "' order by dat desc, tim desc limit 500;")
    result = mycursor.fetchall()
    result.reverse()
    for row in result:
        x_train.append(N + 1)
